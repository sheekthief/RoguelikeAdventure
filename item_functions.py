...
from components.ai import ConfusedMonster, Frozen
...
def cast_heal(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and not entity.ai:
            entity.fighter.heal(amount)
            results.append({'consumed': True, 'message': Message('{0} is healed for {1}!'.format(entity.name, amount), libtcod.green)})

        break

    else:
        results.append({'consumed': False, 'message': Message('There is no targetable friend at that location.', libtcod.yellow)})
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({'consumed': True,
                            'message': Message('The ice bolt connects! {} is frozen!'.format(entity.name, damage), libtcod.green)})
